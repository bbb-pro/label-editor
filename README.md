...
左侧工具箱提供：**文本 · 段落 · 矩形 · 椭圆 · 直线 · 图片 · 条码 · 二维码**，点击即在画布中心创建。

- **文本**：双击编辑文字；拉伸文本框只改变框尺寸，**字号不变**。属性面板支持**字间距**（pt，负值收紧 / 正值拉开），PDF 与打印会按同一字距输出。
- **段落文本**：定宽的多行区域文本框，拖左右两侧控制点调整段落宽度，**文字自动换行并适应边界**；高度默认贴合文字内容。外观与普通文本一致（无底色、无边框），需要可见边界时在属性面板开「区域框」——该框仅作排版参考，**不会被打印或导出**。
- **图片**：从本地选择图片文件插入。
