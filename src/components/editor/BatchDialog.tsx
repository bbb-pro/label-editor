...
export default function BatchDialog({
  ...
  const valid = Number.isFinite(parsed) && parsed >= 1 && parsed <= maxCopies

  // 实时预览首/末序号（无序列化配置时不显示）
  const preview = (() => {
    if (!serial?.enabled || !valid) return null
      ...
      <DialogContent className="sm:max-w-sm">
        ...
        <div className="space-y-2 py-2">
          <label className="flex items-center gap-2">
            <span className="w-16 shrink-0 text-xs text-muted-foreground">打印份数</span>
            <input
              type="number"
              min={1}
              max={maxCopies}
